...
impl<L: Language> PartialRecExpr<L> {
    ...
    fn select_next_open<R: Rng>(&self, rng: &mut R) -> Option<(Id, Trace)> {
        match self {
    ...
    fn fill_next(&mut self, trace: Trace, pick: &L) -> Result<(), SampleError> {
...
impl<L: Language> TryFrom<PartialRecExpr<L>> for RecExpr<L> {
    ...
    fn try_from(partial_rec_expr: PartialRecExpr<L>) -> Result<Self, Self::Error> {
        fn rec<LL: Language>(
            ...
            let PartialRecExpr::Filled { children, mut pick } = partial_rec_expr else {
