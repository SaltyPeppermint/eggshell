...
pub(crate) fn eq() -> Vec<Rewrite> {
    vec![
        ...
        rw!("Eq-a-b"        ; "(== (* ?a ?x) ?b)"    => "0" if halide::compare_constants("?b", "?a", "!%0")),
        rw!("Eq-max-c-pos"  ; "(== (max ?x ?c) 0)"   => "0" if halide::is_const_pos("?c")),
        rw!("Eq-max-c-neg"  ; "(== (max ?x ?c) 0)"   => "(== ?x 0)" if halide::is_const_neg("?c")),
        rw!("Eq-min-c-pos"  ; "(== (min ?x ?c) 0)"   => "0" if halide::is_const_neg("?c")),
