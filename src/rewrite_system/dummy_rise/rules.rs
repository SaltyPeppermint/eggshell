...
use super::{DummyRiseAnalysis, DummyRiseLang, EGraph, Rewrite, substitute};
...
fn contains_ident(v1: Var, v2: Var) -> impl Fn(&mut EGraph, Id, &Subst) -> bool {
...
fn is_const(v: Var) -> impl Fn(&mut EGraph, Id, &Subst) -> bool {
    move |egraph, _, subst| {
        let e: &[DummyRiseLang] = egraph[subst[v]].data.beta_extract.as_ref();
        (e.len() == 1) && matches!(e[0], DummyRiseLang::Symbol(_) | DummyRiseLang::Number(_))
    }
}

pub fn rules(use_explicit_subs: bool) -> HashMap<Symbol, Rewrite> {
    let common = vec![
        // Comment out to deal with unexpected new symbols
        // algorithmic
        rewrite!("map-fusion";
    ...
    let mut map: HashMap<Symbol, _> = common.into_iter().map(|r| (r.name, r)).collect();
    if use_explicit_subs {
        map.extend(explicit_substitution.into_iter().map(|r| (r.name, r)));
    } else {
        map.extend(extraction_substitution.into_iter().map(|r| (r.name, r)));
    }
    map
}

pub fn filtered_rules(names: &[&str], use_explicit_subs: bool) -> Vec<Rewrite> {
    let mut all_rules = rules(use_explicit_subs);
    names
        .iter()
        .map(|&n| all_rules.remove(&Symbol::new(n)).expect("rule not found"))
        .collect()
}
...
// impl Applier<RiseLang, RiseAnalysis> for BetaApplier {
//     fn apply_one(
//         &self,
//         egraph: &mut EGraph,
//         eclass: Id,
//         subst: &Subst,
//         _searcher_ast: Option<&PatternAst<RiseLang>>,
//         _rule_name: Symbol,
//     ) -> Vec<Id> {
//         let new_id =
//             substitute::substitute_eclass(egraph, subst[self.v], subst[self.e], subst[self.body]);
...
impl Applier<DummyRiseLang, DummyRiseAnalysis> for BetaExtractApplier {
    fn apply_one(
        &self,
        egraph: &mut EGraph,
        eclass: Id,
        subst: &Subst,
        _searcher_ast: Option<&PatternAst<DummyRiseLang>>,
        _rule_name: Symbol,
    ) -> Vec<Id> {
        ...
        let v_sym = {
            ...
            assert!(ns.len() == 1, "expected symbol, found {ns:?}");
            match &ns[0] {
                &DummyRiseLang::Symbol(sym) => sym,
                n => panic!("expected symbol, found {n:?}"),
            }
        };
        let result = substitute::substitute_expr(v_sym, ex_e, ex_body);
...
struct MakeFresh {
    prefix: String,
    fresh: Var,
    pattern: Pattern<DummyRiseLang>,
}

impl Applier<DummyRiseLang, DummyRiseAnalysis> for MakeFresh {
    fn apply_one(
        &self,
        egraph: &mut EGraph,
        eclass: Id,
        subst: &Subst,
        searcher_ast: Option<&PatternAst<DummyRiseLang>>,
        rule_name: Symbol,
    ) -> Vec<Id> {
        let sym = DummyRiseLang::Symbol(format!("{}{}", self.prefix, eclass).into());
        let mut new_subst = subst.clone();
        new_subst.insert(self.fresh, egraph.add(sym));
        self.pattern
...
struct CaptureAvoid {
    ...
    v2: Var,
    e: Var,
    if_not_free: Pattern<DummyRiseLang>,
    if_free: Pattern<DummyRiseLang>,
}

impl Applier<DummyRiseLang, DummyRiseAnalysis> for CaptureAvoid {
    fn apply_one(
        &self,
        egraph: &mut EGraph,
        eclass: Id,
        ...
        if v2_free_in_e {
            let mut new_subst = subst.clone();
            let sym = DummyRiseLang::Symbol(format!("_{eclass}").into());
            new_subst.insert(self.fresh, egraph.add(sym));
            self.if_free
                .apply_one(egraph, eclass, &new_subst, searcher_ast, rule_name)
        } else {
