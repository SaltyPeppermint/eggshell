...
impl<L, N> CommutativeSemigroupAnalysis<L, N> for ExprCountFloat
where
    ...
    N::Data: Debug + Sync,
{
        ...
        Self: 'a,
    {
        ...
        rec(&children_counts, 1, 1.0, &mut counts, self.limit);
        counts
    }
...
mod tests {
    ...
    fn simple_term_size_count() {
        ...
        ExprCountFloat::new(10).one_shot_analysis(&egraph, &mut data);

        let root_data = &data[&egraph.find(apb)];

        assert_eq!(root_data[&5], 1.0);
    }
    ...
    fn halide_count_size() {
        ...
        let mut data = HashMap::new();
        ExprCountFloat::new(16).one_shot_analysis(egraph, &mut data);

        let root_data = &data[&egraph.find(root)];

        assert_eq!(root_data[&16], 40512.0);
    }
}
