# This file is automatically generated by pyo3_stub_gen
# ruff: noqa: E501, F401

import builtins
import eggshell
import typing

class GeneratedRecExpr:
    r"""
    Wrapper type for Python
    """
    used_tokens: builtins.int
    def __new__(cls, token_list:typing.Sequence[builtins.str], token_probs:typing.Optional[typing.Sequence[builtins.float]]=None) -> GeneratedRecExpr: ...
    def to_data(self) -> TreeData: ...
    def __str__(self) -> builtins.str: ...
    def __repr__(self) -> builtins.str: ...
    def tree_distance(self, other:GeneratedRecExpr) -> builtins.int: ...
    def to_dot(self, name:builtins.str, path:builtins.str, marked_ids:typing.Optional[typing.Sequence[builtins.int]]=None, transparent:builtins.bool=False) -> None: ...
    @staticmethod
    def count_expected_tokens(token_list:typing.Sequence[builtins.str]) -> builtins.int: ...
    def lower(self) -> RecExpr: ...

class RecExpr:
    r"""
    Wrapper type for Python
    """
    def __new__(cls, s_expr_str:builtins.str) -> RecExpr:
        r"""
        Parse from string
        """
    def __str__(self) -> builtins.str: ...
    def __repr__(self) -> builtins.str: ...
    def to_dot(self, name:builtins.str, path:builtins.str, marked_ids:typing.Optional[typing.Sequence[builtins.int]]=None, transparent:builtins.bool=False) -> None: ...
    def tree_distance(self, other:RecExpr) -> builtins.int: ...
    def arity(self, position:builtins.int) -> builtins.int: ...
    def to_data(self) -> TreeData: ...
    @staticmethod
    def from_data(tree_data:TreeData) -> RecExpr: ...

def eqsat_check(start:RecExpr, goal:RecExpr, iter_limit:typing.Optional[builtins.int]=None, node_limit:typing.Optional[builtins.int]=None, guides:typing.Sequence[RecExpr]=[]) -> tuple[builtins.int, builtins.str, builtins.str]: ...

def first_miss_distance(ground_truth:RecExpr, generated:GeneratedRecExpr) -> FirstErrorDistance: ...

def many_eqsat_check(starts:typing.Sequence[RecExpr], goals:typing.Sequence[RecExpr], iter_limit:typing.Optional[builtins.int]=None, node_limit:typing.Optional[builtins.int]=None, guides:typing.Sequence[typing.Sequence[RecExpr]]=[]) -> builtins.list[tuple[builtins.int, builtins.str, builtins.str]]: ...

def name_to_id(s:builtins.str) -> typing.Optional[builtins.int]: ...

def num_symbols() -> builtins.int: ...

def operators() -> builtins.list[builtins.str]: ...

